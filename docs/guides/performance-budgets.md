...
## Layer compositor on the Scale row (PX5, 2026-09-18)

The N-layer compositor (`preview/engine/layer-compositor.ts`) is budgeted on one worst-case
timeline, the **Scale row**: 3 minutes, 4K, 4 picture layers + text + a 4K matte, generated by
...
| **Export with masks + 4K matte ≤ 1.5× without** (P13)                | `export_video` at 4K, 120–180-frame window                                                                                     | 1.98× → 1.49× / 1.56× CI (PX5.2) → **1.32–1.45×** on CI after PX5.4's exact cuts (CPU 1.20–1.26×); **full 3-minute row 1.32× wall / 1.31× CPU** (PX5.11, CI run 35369857493) | holds on the whole row (CI) |
...
key stack and decode rings; a dropped-frame ledger that _counts_ the project frame index due on
each tick against the last one presented; and peaks for the picture cache, the GL pools and the
live decoders. `tests/e2e/specs/preview-scale-perf.spec.ts` plays and seeks the row through the
real transport and reads only that.
...
~7%). The full 3-minute row did not fit the local memory watchdog while other jobs ran; PX5.11
measured it on a CI runner instead, in the dispatch-only `preview-perf-full.yml` workflow (330-minute
timeout, fixture generated in the job): 3,386 s plain, 4,476 s with the matte, **1.32× wall and
1.31× CPU** over 5,400 4K frames (`PX5-BUDGETS.md`, "PX5.11"). Dispatch it with
`gh workflow run preview-perf-full.yml --ref <branch>`; it takes about 2 h 15 min.
...
20). The review had named the highlight box; the slowest was the grape icon, about 3× the box,
because its ~40 circles flatten to ~740 stroke joints and Pillow's `ImageDraw.line(joint="curve")`
drew each as a pieslice plus two patch lines, from Python: 14.2 / 36–41 ms here, 44.8 / 100.4 ms on
the CI runner, where the 4K ceiling failed. Three changes took it to 3.0 / 8.7 ms. Each round join
is now the whole disc the pieslice was cut from (one ellipse call), which also fills the hairline
cracks those pieslices left through wide curved strokes: no pixel moved outside a stroke, none lost
more than 2 of its 16 supersamples, and every larger change is a crack filled. The composite looks
each pixel up in a table of every coverage combination of one or two coloured parts instead of
blending it in floating point, and a many-piece fill or a dashed or dotted stroke draws its pieces
into one mask instead of one each; both are byte-identical. The slowest shape is now the “NEW”
burst label (3.8 / 13.7 ms), whose time is its label drawn as text at the supersampled size; the
grape icon stays in the guard as the heaviest stroke. The heaviest style a user can reach at that
size is a badge given a stroke as well (fill, stroke and label), which still blends its three parts
directly: about 8 / 30 ms, inside the budget, and the next lever if a heavier shape arrives. The
guard times CPU rather than wall time because the raster is single-threaded (the two agree within
0.2 ms on a quiet machine) while CI runs the suite on three workers, and pauses coverage because the
tracer alone added a quarter to the grape icon's time. Re-run the scan when the catalogue or the
rasteriser changes, and pin whatever is slowest.
