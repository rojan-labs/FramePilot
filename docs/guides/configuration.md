...
| `PEXELS_API_KEY`                    | Pexels credential for stock photo and video search (Settings → AI → Photos & videos (Pexels)). Free and instant from pexels.com/api. Usually stored through Settings; this variable is a fallback for headless setups. Only your search text is sent — see `docs/guides/stock-sourcing.md`. |
