...
| `get_project_state`, `get_timeline`, `get_timeline_summary`, `get_clips`, `get_clip`, `get_transcript`, `get_selected_range`, `list_assets`, `search_elements`                                                                                                                                                                           | read     | Inspect the project (`get_timeline_summary`/`get_clips`/`get_clip` are compact, windowed reads for long-form projects; `get_project_state` returns the media bin as an `assetSummary` tally — use `list_assets` for asset ids) |
| `trim_clip`, `split_clip`, `delete_range`, `ripple_delete`, `delete_clip`, `delete_clips`, `move_clip`, `add_clip`, `add_clips`, `add_track`, `remove_track`, `move_track`, `add_text_layer`, `add_shape`, `set_shape_style`, `add_caption_layer`, `add_keyframes`, `apply_color_grade`, `adjust_audio`, `add_transition`, `track_object` | mutate   | Edit the timeline; each returns a validated, reversible patch that is applied                                                                                                                                                  |
...
Tools whose engine does not exist yet are **not** exposed until their capability lands — the
server never fakes a result. The masking tools (`create_mask`, `remove_background`, …) are
desktop-only: they run in Capability Pack workers the standalone server has no route to.
...
| `FRAMEPILOT_PYTHON_API_URL`     | no       | Base URL of the running [Python render sidecar](../api/python-engine-api.md) (e.g. `http://127.0.0.1:8765`). Without it, `render_preview`/`export_video` report that rendering is unavailable; all editing still works.                                                                                                                              |
...
- _Rendering reports unavailable_ — set `FRAMEPILOT_PYTHON_API_URL` and start the
  ...
  [Python engine API guide](../api/python-engine-api.md)).
