...
Tools whose engine does not exist yet (`generate_mask` — a segmentation is a bitmap, while
timeline masks steer by rectangle bounds) are **not** exposed until their capability lands —
the server never fakes a result.
