...
| `get_project_state`, `get_timeline`, `get_timeline_summary`, `get_clips`, `get_clip`, `get_transcript`, `get_selected_range`, `list_assets`                                                                                                                                                                           | read     | Inspect the project (`get_timeline_summary`/`get_clips`/`get_clip` are compact, windowed reads for long-form projects; `get_project_state` returns the media bin as an `assetSummary` tally — use `list_assets` for asset ids) |
