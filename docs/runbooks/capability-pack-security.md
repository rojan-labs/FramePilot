...
| Watchdog | `worker-watchdog.ts` | Footprint ≤ min(pack limit, 0.6 × RAM); 5 min without progress; declared outputs ≤ the byte ceiling, and the whole staging folder (incl. the worker's `windows/`/`scratch/` and host `inputs/`) ≤ free − 1 GB → `resource_exhausted` (split in E2E.6: the worker's own scratch had been counted against the artifact ceiling) |
...
| Hardened decodes | `media/untrusted.py` (`bounded_decode_input_options`), `frame_hashes.py`, `render/mattes.py` (the export's matte probe, packet index and rawvideo decode, BR4.16), `matte-media-inspector.ts` | `file` protocol only, media demuxers only (Matroska forced for `matte.mkv`/`foreground.mkv`), `-max_pixels`, bounded threads; routes one at a time with work-sized deadlines. A concat file named `matte.mkv` naming a sibling by a relative path is the case ffmpeg itself does not refuse (`test_matte_fuzz_media.py`) |
