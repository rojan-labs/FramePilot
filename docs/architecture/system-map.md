      ...
      └─ window.framepilot bridge (apps/desktop/electron/preload.cts) ──── 'framepilot:*' IPC channels
           └─ apps/desktop/electron/main.ts  registerIpcHandlers()  (+ ipc/, ai/, render/, media/, sidecar/, capability-packs/)
                ...
                └─ Python sidecar (engine/python/framepilot_engine/service.py, FastAPI) — spawned by main.ts spawnSidecar()
                      ...
                      ├─ preview rasters: /preview/text-raster (titles, captions and shapes drawn by the export's own code for the monitor)
...
| Elements photos/videos (Pexels) / music       | `stock:search/preview/download/…`, `music:search/preview/download/…`                                                    | `ai/stock-host.ts`; shared by UI and agent (see memory note "agent shares panel services")                                                   |
| Elements stickers                             | `elements:materialize`, `elements:thumbnail`                                                                            | `media/elements-library.ts`: catalogue ids in; the sticker copied into the project as an asset, or packaged tiles as WebP bytes (at most 96 ids a request); no path crosses (ADR 0191)|
