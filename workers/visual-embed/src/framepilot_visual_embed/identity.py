...
Same contract as Subject Intelligence: the trusted installer — which has already verified
the signed catalog, the artifact hash and the platform code signature — passes the
approved identity and capability roster in the environment, and the worker echoes it only
after checking everything it can independently know, including that every pinned weight
hashes to its approved digest.

While the digests are placeholders (see :mod:`framepilot_visual_embed.models`), the health
check FAILS. That is the intended state: a pack that cannot name the weights it loads must
not pass its own health check, and the installer will refuse to register it.
"""
...
from .backend import BackendUnavailableError, ModelUnavailableError, VisualEmbedBackend
