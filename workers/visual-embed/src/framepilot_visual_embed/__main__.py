"""Signed entrypoint for the Visual Embed Capability Pack worker.
...
``--framepilot-health-check``
    ...
    proves every pinned model file hashes to its approved digest.
...
from .backend import VisualEmbedBackend
...
def create_backend() -> VisualEmbedBackend:
    """Import the inference backend lazily so protocol failures never need onnxruntime."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if arguments == [RUNTIME_FLAG]:
        return _runtime()
    sys.stderr.write(
        f"framepilot-visual-embed requires exactly one of {HEALTH_CHECK_FLAG} or {RUNTIME_FLAG}.\n"
    )
    return 2
...
def _health_check() -> int:
    try:
        ...
        sys.stderr.write(f"Visual Embed health check failed: {type(error).__name__}: {error}\n")
        return 1
