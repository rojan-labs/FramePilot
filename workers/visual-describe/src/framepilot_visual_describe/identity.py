...
Same contract as Visual Embed: the trusted installer — which has already verified the
signed catalog, the artifact hash and the platform code signature — passes the approved
identity and capability roster in the environment, and the worker echoes it only after
checking everything it can independently know, including that every pinned weight and the
llama.cpp binary hash to their approved digests.

While the digests are placeholders (see :mod:`framepilot_visual_describe.models`), the
health check FAILS. That is the intended state: a pack that cannot name the model it is
about to run over a customer's footage must not pass its own health check, and the
installer will refuse to register it.
"""
...
from .backend import BackendUnavailableError, DescribeBackend, ModelUnavailableError
...
def build_handshake(
    ...
    try:
        ...
        raise HealthCheckError(f"pinned artifact verification failed: {error}") from error
