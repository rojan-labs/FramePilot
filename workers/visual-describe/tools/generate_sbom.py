"""Generate the Visual Describe SBOM and license notice from the real environment.
...
This pack has one obligation Tracking Lite does not: **the weights AND the llama.cpp
runtime are redistributable artifacts with their own licences**, so every GGUF model and
every runtime binary/dylib appears in the SBOM as a component with its own digest and
licence, and `--check` fails if the compiled-in pins and the lock file ever disagree.
...
Neither this check nor `--check` downloads a single weight or runtime byte: it reads
`pack/models.lock.toml` and the digests compiled into `models.py`, and compares them to
each other. Verifying the *installed* artifacts against their pins is
...
SELF_DISTRIBUTION = "framepilot-visual-describe"
#: Weights and runtime binaries may only carry licences a commercial desktop product can ship.
...
# same list Tracking Lite, Subject Intelligence and Visual Embed already carry.
...
def collect_models() -> list[ModelRecord]:
    """Every pinned weight and runtime artifact named in `pack/models.lock.toml`.

    The lock file names several source tables (`[source]`, `[runtimeSource]`,
    `[smallSource]`, …) and each `[[model]]` entry says which one it came from via its own
    `source` key, defaulting to the table literally named `source`. This reads that
    generically rather than hard-coding the table names, so a new source table needs no
    change here. `[[archive]]` blocks (the whole llama.cpp release tarball, pinned as one
    digest) are packaging metadata, not separately-shipped components — every file inside
    the archive that this pack actually loads already has its own `[[model]]` entry.
    """
...
def verify_models(models: list[ModelRecord]) -> list[str]:
    ...
    from framepilot_visual_describe.models import MODELS_BY_ID

    problems: list[str] = []
    if {model.identifier for model in models} != set(MODELS_BY_ID):
        problems.append("models.lock.toml and the compiled model pins name different artifacts")
        return problems
    for model in models:
        ...
        if model.license_id not in PERMITTED_MODEL_LICENSES:
            problems.append(
                f"{model.file} is {model.license_id}, which is not a permitted artifact licence"
            )
    return problems
...
def build_sbom(
    ...
    # Weights and the llama.cpp runtime are first-class components: a downstream scanner
    # must be able to see everything a signed artifact carries and under what terms.
    for model in models:
        # The runtime binary and its dylibs are ordinary native libraries; the GGUF
        # files are the machine-learning weights.
        component_type = "machine-learning-model" if model.file.endswith(".gguf") else "library"
        entry = {
            "type": component_type,
            "name": model.name,
            "version": model.file,
...
def main() -> int:
    ...
    if arguments.check:
        print(
            ...
            f"{len(models)} artifacts, {len(BUNDLED_NATIVES)} natives"
        )
        return 0
