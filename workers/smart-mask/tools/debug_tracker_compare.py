...
def f(i):
    if i not in feats:
        feats[i] = sam.encode_image(preprocess(frames[i]))
    return feats[i]
...
def attend(curr, curr_pos, memory, memory_pos, valid):
    seen.append((curr, memory, memory_pos, valid))
    return orig(curr, curr_pos, memory, memory_pos, valid)
...
print(
    "cond obj_ptr maxdiff",
    np.abs(cond[0].obj_ptr - ref["cond_obj_ptr"].reshape(-1)).max(),
    "score",
    cond[0].score,
    ref["cond_score"],
)
...
for i, (curr, memory, pos, valid) in enumerate(seen):
    print(
        f"frame {i + 1}: valid same",
        np.array_equal(valid, ref[f"f{i}_valid"]),
        "valid count",
        valid.sum(),
        ref[f"f{i}_valid"].sum(),
    )
    print(
        "  curr",
        np.abs(curr - ref[f"f{i}_curr"]).max(),
        "memory",
        np.abs(memory - ref[f"f{i}_memory"]).max(),
        "pos",
        np.abs(pos - ref[f"f{i}_memory_pos"]).max(),
    )
    d = np.abs(pos - ref[f"f{i}_memory_pos"]).reshape(-1, 64).max(axis=1)
    print("  pos diff tokens >1e-3:", np.nonzero(d > 1e-3)[0][:5], (d > 1e-3).sum())
    d = np.abs(memory - ref[f"f{i}_memory"]).reshape(-1, 64).max(axis=1)
    print("  mem diff tokens >1e-3:", np.nonzero(d > 1e-3)[0][:5], (d > 1e-3).sum())
