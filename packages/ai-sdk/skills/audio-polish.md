...
- **Duck it in the same breath.** `add_music` takes `duckUnderTrackId`; pass the id of the track carrying the sound the bed must stay out of the way of. Usually that is dialogue. It is not always: on a project with no voice — action footage, a wind-and-engine POV — the sound to duck is on the picture track, and a video track is a valid target because the duck follows that track's clips. A bed laid at full level over the thing it is scoring is not a finished mix.
