...
function talkingHeadProject(fps: number): Project {
  return parseProject({
    ...
    assets: [{ id: ASSET, path: 'media/ISOM_Batch1_Assignment1.mp4', kind: 'video', durationSeconds: DURATION }],
...
describe('caption_the_edit against the run 7d159862 transcript', () => {
  ...
  it('gives every cue a clip id of its own', () => {
    ...
    const ids = assembled.patch.operations
      .filter((op): op is Extract<typeof op, { type: 'add_caption_layer' }> => op.type === 'add_caption_layer')
      .map((op) => op.clipId);
