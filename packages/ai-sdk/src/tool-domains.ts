...
export const DOMAIN_SUMMARY: Readonly<Record<Exclude<ToolDomain, 'core'>, string>> = {
  ...
  elements:
    'stickers and emoji, and shapes over the picture — highlight boxes, arrows, circles, markers, underlines, callouts, numbered badges, stars, icons; find, place, restyle and move them',
  footage:
...
const DOMAIN_MEMBERS: Readonly<Record<Exclude<ToolDomain, 'core'>, readonly string[]>> = {
  ...
  elements: ['search_elements', 'add_sticker', 'add_shape', 'set_shape_style'],
  footage: [
    'detect_scenes',
    'search_media',
    'find_similar',
...
const DOMAIN_REQUEST_WORDS: Readonly<Record<Exclude<ToolDomain, 'core'>, RegExp>> = {
  ...
  elements:
    /\b(stickers?|emojis?|shapes?|arrows?|circl(?:e|es|ing)|highlight(?:ed)? box(?:es)?|highlight (?:the|a|an)|underlin\w*|callouts?|badges?|speech bubbles?|box(?:es)? around|point(?:ing)? (?:at|to))\b/gi,
  footage:
