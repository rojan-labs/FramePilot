import { ChatGoogleGenerativeAI } from '@langchain/google-genai';
import type { BaseChatModel } from '@langchain/core/language_models/chat_models';
import { GOOGLE_DEFAULT_MODEL } from './provider-defaults.js';
import type { AiCompletionRequest } from './types.js';
import { OpenAiStyleProvider, baseOptions } from './langchain-provider-shared.js';

export class ConcreteLangChainGoogleProvider extends OpenAiStyleProvider {
  public readonly name = 'google' as const;
  public get modelId(): string {
    return this.config.model ?? GOOGLE_DEFAULT_MODEL;
  }
  protected buildModel(request: AiCompletionRequest, streaming: boolean): BaseChatModel {
    return new ChatGoogleGenerativeAI({
      ...baseOptions(this.modelId, this.config.apiKey, streaming),
      ...(request.temperature !== undefined ? { temperature: request.temperature } : {}),
      ...(request.maxTokens !== undefined ? { maxOutputTokens: request.maxTokens } : {}),
      ...(this.config.baseUrl !== undefined ? { baseUrl: this.config.baseUrl } : {}),
    }) as unknown as BaseChatModel;
  }
}
