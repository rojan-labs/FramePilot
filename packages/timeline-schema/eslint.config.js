...
export default tseslint.config(
  ...
  {
    // Node build scripts (plain ESM, not TS) run under node, so declare its
    // globals — `no-undef` otherwise flags `process` (TS files get this from
    // the type-checker instead).
    files: ['scripts/**/*.mjs'],
    languageOptions: {
