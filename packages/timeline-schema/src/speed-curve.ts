 ...
 */
import type { Clip, SpeedPoint } from './index.js';
import { applyEasing } from './keyframe-curves.js';
 ...
 */
export function integrateRate(points: readonly SpeedPoint[], from: number, to: number): number {
  if (to <= from) return 0;
  const ramp = normalizeRamp(points);
 ...
 */
export function sourceSpanForDuration(clip: Clip, from: number, timelineDuration: number): number {
