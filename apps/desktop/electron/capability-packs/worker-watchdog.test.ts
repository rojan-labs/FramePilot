...
describe('worker watchdog limits and sampling', () => {
  it('takes the smaller of the pack limit and 0.6 x RAM, and of the ceiling and free space minus 1 GB', () => {
  ...
  it('trips once, in order: stall, disk, memory; unmeasurable memory is not a breach', async () => {
    ...
    const watchdog = new WorkerWatchdog({ memoryBytes: 100, stallMs: 1_000, stagingBytes: 50 }, probes, { stagingDirectory: '/s', onBreach });
    watchdog.attach(7);
    await watchdog.tick();
    footprint = 99;
    staged = 50;
    ...
    const stalled = new WorkerWatchdog({ memoryBytes: 100, stallMs: 1_000, stagingBytes: 50 }, probes, { stagingDirectory: '/s', onBreach: vi.fn() });
    now = 5_000;
    stalled.progress();
    now = 6_001;
    await stalled.tick();
