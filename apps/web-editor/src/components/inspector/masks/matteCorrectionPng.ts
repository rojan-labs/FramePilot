 ...
 * chunks, the artifact's exact size, and only the four defined values — **keep = 255, remove = 0,
 * edge = 64, untouched = 128** (edge since BR6.10). A canvas `toBlob('image/png')` produces an RGBA PNG, which that reader
...
/** A soft edge to re-matte (hair, blur): the worker widens its band there; it is not a constraint. */
export const BRUSH_EDGE = 64;
export const BRUSH_UNTOUCHED = 128;

const BRUSH_VALUE: Readonly<Record<CorrectionStroke['kind'], number>> = {
  keep: BRUSH_KEEP,
  remove: BRUSH_REMOVE,
  edge: BRUSH_EDGE,
};
...
export interface CorrectionStroke {
  readonly kind: 'keep' | 'remove' | 'edge';
  readonly radiusPx: number;
  readonly points: readonly { readonly x: number; readonly y: number }[];
}

/**
 * Paint strokes into a grayscale buffer: keep = 255, remove = 0, edge = 64, everything else
 * untouched. Later strokes overwrite earlier ones where they cross.
 *
 * Drawn here rather than on a canvas because a canvas would antialias, and a value between the
 * four the host allows is refused — an antialiased edge would fail the whole save.
 */
export function paintCorrection(
  ...
  for (const stroke of strokes) {
    const value = BRUSH_VALUE[stroke.kind];
