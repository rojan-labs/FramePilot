 ...
 *
 * PX5.3: and the monitor tier's `planes.mkv` (`render/matte_tier.py`), which replaces the 4K
...
async function measure(
  file: string,
  size: readonly [number, number] = [3840, 2160],
): Promise<{ format: string; p50: number; p95: number }> {
  ...
  expect([info.width, info.height]).toEqual(size);
  ...
  it.skipIf(!existsSync(MANIFEST))(
    ...
    async () => {
      ...
      for (const name of ['matte.mkv', 'foreground.mkv']) {
        ...
        rows.push({ name, size: '3840x2160', format, p50: p50.toFixed(1), p95: p95.toFixed(1) });
      }
      if (manifest.tier !== undefined) {
        const [width, height] = manifest.tier.size;
        const planes = path.join(FIXTURE, manifest.tier.root, manifest.matte.key, 'planes.mkv');
        // Eight byte planes: each 16-bit plane's high then low bytes (`matte_tier.split_bytes`).
        const { format, p50, p95 } = await measure(planes, [width, 8 * height]);
        const size = `${width}x${8 * height}`;
        rows.push({
          name: 'tier planes.mkv',
          size,
          format,
          p50: p50.toFixed(1),
          p95: p95.toFixed(1),
