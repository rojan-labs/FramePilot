...
class EntityRow(BaseModel):
    """One tier-1 identity cluster persisted in ``entities`` (ADR 0175, VU5.3).

    A person (or a setting) the ledger has recognised across shots. ``id`` is generated by
    :mod:`framepilot_engine.brain.entities` from first appearance and never renumbered;
    ``label`` is the one HUMAN-authored value in the whole ledger ("person_03" → "Marcus")
    and a re-cluster must preserve it, which is why an existing id survives a merge.

    ``centroid`` is packed by :func:`~framepilot_engine.brain.embeddings.pack_vector` and
    stored beside its ``dim`` and producing ``model`` — the same "never mix two models'
    vectors" discipline ``visual_vectors`` follows. A face embedded by one pack must never
    be compared against a centroid built by another.
    """

    id: str = Field(description="Stable cluster id, e.g. person_03.")
    kind: str = Field(description="person | setting")
    label: str | None = Field(
        default=None, description="Human-authored name; never written by a model."
    )
    centroid: list[float]
    dim: int = Field(gt=0)
    model: str = Field(description="Producing model id; two identity spaces never mix.")
