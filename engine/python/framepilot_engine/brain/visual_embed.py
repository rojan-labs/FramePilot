...
class VisualEmbedClient:
    ...
    def embed_passages(self, images: Sequence[bytes]) -> EmbedResult:
        ...
        for start in range(0, len(data_uris), self._batch_size):
            vectors.extend(self._embed_batch(data_uris[start : start + self._batch_size]))
        return EmbedResult(model=MODEL_ID, dim=self._dim, vectors=vectors)
    ...
    def _post(self, payload: dict[str, object]) -> dict[str, object]:
        ...
        """
        while True:
            key = self._ring.acquire(self._now())
            if key is None:
        ...
        for vector in vectors:
            if self._dim is None:
                ...
                raise VisualEmbedError(
                    ...
                    f"captured dimension {self._dim} for {MODEL_ID}."
                )
        return vectors
