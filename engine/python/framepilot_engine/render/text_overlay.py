...
Only ``text`` is persisted by the current authoring UI/tools (position/style
are documented as preview-only presentation with no schema field yet — see
``OverlaysPanel.tsx``), so the deterministic baseline here draws exactly that:
white text, centered in the frame, with a thin outline for legibility over any
background. Optional ``fontSize`` (pixels) and ``color`` (``#rrggbb`` /
``#rrggbbaa``) keys already flow through the generic ``Effect.params`` bag via
``set_effect_params`` and are honored when present, so a future style-authoring
UI needs no further render change.
...
def text_overlay_style(
    ...
    :param params: The ``text`` effect's ``params`` (``text`` plus optional
        ``fontSize``/``color``).
    :param frame_height: Target frame height, used to scale the default font size.
...
def render_text_overlay_image(
    ...
    *,
    ...
    max_text_width = int(frame_width * _MAX_WIDTH_FRACTION)
