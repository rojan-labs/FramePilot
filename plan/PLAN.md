...
- `[x]` GOLDEN-F.5 — a run that throws shows a plain sentence with the next action (`reliability/plain-failure.ts`), raw text in `detail`; every message passes the harness's failure-quality predicate.
