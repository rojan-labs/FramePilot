...
- [x] U4.1 — reasoning effort follows the run stage (`low` executing a locked plan, `medium` deciding/repairing/recovering) — `e9066c5b`; `agentStepReasoningEffort` in `kernel/stage-policy.ts`; quality at `low` not yet measured (U4.4)
...
- [x] X1 — a transition or caption edit verifies itself in its result (`kernel/verification-note.ts`); `verify_*` stay for a re-check and say so — PR #122. Evidence: TRACKING.md §X1, 14 verification-only steps (188 s) since 09-01
- [x] X2 — the Agent SDK provider pre-spawns the next `claude` process while a step's tools run (`FRAMEPILOT_AGENT_SDK_PREWARM`, idle 60 s, tool-bearing calls only) — PR #122. Measured: first stream event 2.2 s cold → 1.3 s warm
- [x] X3 — the repair pass and the classifier name their thinking effort (`medium` / `low`), the Agent SDK adapter defaults unnamed requests to `medium` instead of the SDK's `high`, and the repair call emits its context-usage pair — PR #122. Evidence: TRACKING.md §X4 (26 s and 12 s silent repair passes)
...
- [~] PX0–PX5 — inventory, frame plan, pixel oracle, N-layer compositor, delete gates, perf budgets (PX0, PX1, PX3, PX4 done; PX2 51/52 oracle rows, the matte row waits on BR5; PX5 pending)
...
- [~] BR2–BR7 — (BR2 done) matte engine, Smart Mask pack, protocol + host, preview matte, UI + review, matte gates
