...
**52 cases.** WebCodecs canvas: 21 · DOM PreviewPlayer: 31 · WebCodecs, overlays disabled: 0 · blank: 0
...
| --- | --- | --- | --- | --- | --- |
| Alpha: clip mask stack (v22 rectangle/ellipse/path, static and keyframed) | `alpha/mask-shapes` | DOM PreviewPlayer | canvas gate: stacked pictures the front clip does not hide (ADR 0169/0170) | export stacks up to 2 pictures; DOM player shows only the front-most active clip (PreviewPlayer.tsx videoLocation)<br>clip mask: DOM draws the stack raster on the one visible clip only (maskRasterCssImage) | passes (min PSNR Infinity dB, min 100% within 8/255) |
| Alpha: mask stack combine modes (add/subtract/intersect/difference/lighten/darken; rectangle, ellipse and path) | `alpha/mask-modes` | DOM PreviewPlayer | canvas gate: stacked pictures the front clip does not hide (ADR 0169/0170) | export stacks up to 2 pictures; DOM player shows only the front-most active clip (PreviewPlayer.tsx videoLocation)<br>clip mask: DOM draws the stack raster on the one visible clip only (maskRasterCssImage) | passes (min PSNR Infinity dB, min 100% within 8/255) |
| Alpha: mask edges (invert, expansion, inner/outer feather, three falloffs, per-vertex feather) | `alpha/mask-edges` | DOM PreviewPlayer | canvas gate: stacked pictures the front clip does not hide (ADR 0169/0170) | export stacks up to 2 pictures; DOM player shows only the front-most active clip (PreviewPlayer.tsx videoLocation)<br>clip mask: DOM draws the stack raster on the one visible clip only (maskRasterCssImage) | passes (min PSNR Infinity dB, min 100% within 8/255) |
| Alpha: effect-target masks (a grade limited to a keyframed ellipse minus a rectangle, under an alpha cut) | `alpha/mask-effect-target` | DOM PreviewPlayer | canvas gate: stacked pictures the front clip does not hide (ADR 0169/0170) | export stacks up to 2 pictures; DOM player shows only the front-most active clip (PreviewPlayer.tsx videoLocation)<br>clip mask: DOM draws the stack raster on the one visible clip only (maskRasterCssImage) | passes (min PSNR Infinity dB, min 100% within 8/255) |
| Alpha: gaussian-legacy masks (feathered rectangle, inverted ellipse, polygon in a stack) | `alpha/mask-legacy` | DOM PreviewPlayer | canvas gate: stacked pictures the front clip does not hide (ADR 0169/0170) | export stacks up to 2 pictures; DOM player shows only the front-most active clip (PreviewPlayer.tsx videoLocation)<br>clip mask: DOM draws the stack raster on the one visible clip only (maskRasterCssImage) | passes (min PSNR Infinity dB, min 100% within 8/255) |
