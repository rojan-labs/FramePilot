...
| Alpha       | mask stack: every kind (rectangle, ellipse, path, matte, key) × every mode × invert × expansion × inner/outer and per-vertex feather × legacy feather; effect-target masks; tracked masks (each method); matte × shape; opacity keyframes; letterboxed sources; PNG with alpha; `linear`, `band`, `gradient`, `layer` (track matte, text as mask) kinds; masks on adjustment lanes (`space: 'frame'`); edge-style effects; progressive partially processed mattes |
| Geometry    | transform keyframes, crop, fit/letterbox across aspects, rotation metadata, **anamorphic pixel aspect ratio**, portrait phone footage                                                                                                                                                                                                                                                                                                                             |
| Time        | constant speed 0.25×–4×, speed ramps, reverse if supported, trims at non-keyframes, VFR source, edit-list source, **mixed frame rates on one timeline**, **interlaced sources** (engine deinterlace policy)                                                                                                                                                                                                                                                       |
