...
6. **Faces are recognised only with consent.** Identity matching for requests like "everyone except the
   host" needs the per-project opt-in (biometric processing; see `12` P15). Without it the agent returns
   `needs_face_selection`, the sidebar shows the detected faces as thumbnails, and the editor picks.
...
| `mask_with_layer`         | `clipId`, `sourceClipId`, `channel`                                                                                                                                                                                                                                                                    | Track matte / text-as-mask (`layer` kind)                                                                                                                                                                                                 |
| `create_shape_mask`       | `clipId`, `preset: 'split' \| 'mirror' \| 'gradient' \| 'heart' \| 'star' \| …`, `placement: candidateId \| 'frame_center' \| user numbers`, `purpose`                                                                                                                                                 | Presets and analytic kinds placed from a candidate or the frame, never free coordinates                                                                                                                                                   |
| `follow_subject`          | `clipId`, `maskId` or `candidateId`, `targetClipId` (text/overlay)                                                                                                                                                                                                                                     | `use_track`: a title or graphic follows the tracked subject                                                                                                                                                                               |
