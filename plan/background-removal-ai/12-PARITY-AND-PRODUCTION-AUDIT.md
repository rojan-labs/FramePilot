...
| P17 | Worker decodes untrusted media (FFmpeg attack surface)                                                                                                                      | Medium      | No OS sandbox, and a malicious pack can escape on every OS (POSIX `setsid()` leaves the group; Windows has no Job Object and checks only the worker pid). Contains faulty workers only: network disabled by protocol; own process group killed on timeout, abort and completion; host watchdog for memory, stalled progress and staging size; host verification and a pre-rename re-check; fuzzed-media tests (BR4.12; accepted risk in ADR 0114)                                                                                                             | `03`       |
