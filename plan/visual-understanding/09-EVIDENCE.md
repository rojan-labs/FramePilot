...
## VU5 / VU7 / VU9 (2026-09-07)

| Claim                                           | Result                                                                                                                                                                                                                                                                           | Where                               |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| VU7's verification reaches the model            | a real `streamAgent` run over a gapped two-clip timeline: turn 1 trims the gap shut, manufacturing one cut with an exposure jump (ledger luma 0.15 vs 0.85); the verification fact appears in the NEXT turn's prompt with its `picture:` citation, and never as a review finding | `picture-verification.run.test.ts`  |
| …and costs nothing when there is no ledger      | the identical run emits a **byte-identical** event sequence with the same applied-op count                                                                                                                                                                                       | same                                |
| VU7's token cost                                | **0** — no prompt-visible string changed; no golden regenerated                                                                                                                                                                                                                  | —                                   |
| The pairwise duplicate scan is gone             | `grep -rn "_SIMILAR_GROUP_SPAN_CAP" engine/python` returns nothing; the multi-index bucket is exact by pigeonhole and checked against an all-pairs scan over 150 seeded hashes                                                                                                   | `test_duplicates.py`                |
| The golden gate enforces the perception ceiling | against the real recorded baseline the rows read "n/a — not measured"; against a synthetic run at frames 0.00 → 1.40 and guess rate 20% → 90%, **both trip as REGRESSION**                                                                                                       | `golden-gate.mjs`, proven both ways |
| Licence scan                                    | 7 packages, no denylisted licences — **and it clears nothing here**: the script walks `node_modules` manifests only and cannot see a Python distribution, a native binary or a weight                                                                                            | `scripts/license-scan.mjs`          |

## What CI caught that targeted runs did not (2026-09-07)

Recorded because the pattern repeated four times, and each time it looked like a one-off.

| Round | Failure                                                                                                            | Why a targeted run missed it                                                                         |
| ----- | ------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- |
| 1     | `eslint`: an import and a helper orphaned by deleting `renderPictureBriefing`                                      | lint is package-scoped; the module was linted, its test was not                                      |
| 2     | `mypy`: three stale `type: ignore`s, one masking a real error                                                      | `mypy .` covers `tests/`; it had only been run on modules                                            |
| 3     | `pytest`: `test_ai_tools.py` asserts the registry's whole tool SET against a flags contract                        | nothing in the filename suggests it enumerates the registry                                          |
| 4     | `pytest`: `test_tool_registry_schema_parity.py` — `search_visual`'s `facts` filter existed in TS and not in Python | the two contracts had silently diverged; found by running the full engine suite (3,118 tests, 2m16s) |

**The rule:** for a change touching a registry, a schema or any shared contract, "the tests
your change touches" is not knowable from the diff. Targeted runs for logic; one full suite
run for contracts.
